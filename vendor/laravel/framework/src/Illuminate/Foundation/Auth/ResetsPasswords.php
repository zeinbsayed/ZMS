...
trait ResetsPasswords
{
    ...
    public function sendResetLinkEmail(Request $request)
    {
		...
		$user=User::where('email',$request->only('email'))->select('id')->get();
		//dd($user);
		if(isset($user[0])){
			if(isset($user[0]->id))
			{
				return redirect()->action('Auth\PasswordController@getReset',array('token'=>$request["_token"]));
			}
			else{
    ...
    public function reset(Request $request)
    {
		...
		if(isset($user[0]))
		{
			...
			$this->resetPassword($user,$password['password']);
